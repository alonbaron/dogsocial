...
public class PlaydateService {
  ...
  public PlaydateDtos.PlaydateResponse create(PlaydateDtos.CreatePlaydateRequest req) {
    ...
    User recipient = userRepository.findByUsername(req.getRecipientUsername())
        .orElseThrow(() -> new NotFoundException("User @" + req.getRecipientUsername() + " not found"));

    if (me.equals(recipient.getId())) {
